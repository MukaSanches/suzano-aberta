...
def make_db(path: Path) -> None:
    ...
    CREATE TABLE records (
      ...
      content_hash TEXT NOT NULL, payload_json TEXT NOT NULL, active INTEGER NOT NULL DEFAULT 1
    );
    """)
    payload = {
        "id": "proposicao:1", "kind": "proposicao", "title": "Educação em Suzano",
        "summary": "Transporte escolar", "date": "2026-09-15", "year": 2026,
        "attributes": {"tema": "educação pública"},
        "source": {"name": "Fonte de teste", "url": "https://example.test/item"},
    }
    conn.execute(
        "INSERT INTO records VALUES (?,?,?,?,?,?,?,?,?,1)",
        ("proposicao:1", "proposicao", "Educação em Suzano", "Fonte de teste", "https://example.test/item", "2026-09-15", "2026-09-15", "abc", json.dumps(payload, ensure_ascii=False)),
    )
    conn.commit(); conn.close()


def test_build_web_dataset(tmp_path: Path) -> None:
    database = tmp_path / "test.sqlite3"; make_db(database)

...
CPTM_STATUS_URLS = ("https://www.cptm.sp.gov.br/cptm", "https://www.cptm.sp.gov.br/Pages/Home.aspx?origem=menu")
...
STATUSES = (
    "Operação Normal", "Operação Parcial", "Operação Especial", "Operação Diferenciada",
    "Operação Transitória", "Operação com Impacto Pontual", "Impacto Pontual",
    "Velocidade Reduzida", "Maiores Intervalos", "Operação Paralisada", "Paralisada",
    "Atividade Programada", "Circulação de Trens", "Dados Indisponíveis",
    "Dados/Status Indisponíveis", "Status Desconhecido", "Status não disponível", "Operação Encerrada",
)
STATUS_RE = "|".join(sorted(map(re.escape, STATUSES), key=len, reverse=True))
LINE_RE = re.compile(rf"(?:Linha\s*11\s*[-–—]?\s*Coral|CORAL)\s*(?:[:\-–—]\s*)?({STATUS_RE})", re.I)
UPDATED_RE = re.compile(r"Atualizado\s+em\s*:?\s*(\d{2}/\d{2}/\d{4}\s+\d{2}:\d{2}(?::\d{2})?)", re.I)
SEGMENT_RE = re.compile(r"entre\s+(?:as\s+)?esta(?:ç|c)[õo]es?\s+([A-Za-zÀ-ÿ0-9 .'\-–—]+?)\s+e\s+([A-Za-zÀ-ÿ0-9 .'\-–—]+?)(?=\s*(?:,|\.|devido|por|com|$))", re.I)
...
def _source(source_id: str, name: str, url: str, role: str, *, structured: bool, requires_key: bool = False, live: bool = True, note: str | None = None) -> dict[str, Any]:
    data: dict[str, Any] = {"id": source_id, "name": name, "url": url, "role": role, "structured": structured, "requires_api_key": requires_key, "eligible_for_live_status": live, "verified_official": True}
    if note:
        data["note"] = note
    return data
...
VERIFIED_SOURCES = (
    _source("cptm-status", "CPTM — Situação das Linhas", CPTM_STATUS_URLS[0], "status_operacional", structured=False, note="Aceito somente quando a Linha 11/CORAL e um estado reconhecido aparecem explicitamente."),
    _source("artesp-api-trilhos", "ARTESP — API Trilhos", ARTESP_DOCS_URL, "status_regulatorio", structured=True, requires_key=True, note="Desde 03/09/2026 a documentação declara escopo somente para linhas sob regulação direta da ARTESP."),
    _source("artesp-status-publico", "ARTESP — Status das Linhas", ARTESP_PUBLIC_STATUS_URL, "status_regulatorio_fallback", structured=False, note="Aceito somente se a Linha 11 aparecer explicitamente."),
    _source("cptm-ocorrencias-pdf", "CPTM — Comunicado de Ocorrência", CPTM_OCCURRENCE_DOWNLOAD_URL, "historico_documental", structured=False, live=False, note="Download oficial para ID já conhecido; não é usado como feed ao vivo porque não foi localizada documentação oficial de listagem pública."),
)
        ...
        if isinstance(companies, list):
            for company in companies:
                ...
                for line in company.get("linhas") or []:
                    if isinstance(line, dict) and (str(line.get("codigo") or "") == "11" or "linha 11" in _fold(line.get("nome") or line.get("linha"))):
                        return line, str(company.get("nome") or "").strip() or None
        for key, value in payload.items():
            if key not in {"meta", "status", "classificacao"}:
                found = _line11(value)
                if found:
                    return found
...
def _occurrences(payload: Any) -> list[dict[str, Any]]:
    ...
    if isinstance(payload, dict):
        ...
        code = str(line.get("codigo") or "") if isinstance(line, dict) else str(payload.get("linha_codigo") or "")
        name = str(line.get("nome") or "") if isinstance(line, dict) else str(line or payload.get("linha_nome") or "")
        if (code == "11" or "linha 11" in _fold(name) or "11-coral" in _fold(name)) and any(k in payload for k in ("descricao", "situacao", "data_hora", "timestamp")):
...
def _segment(text: str) -> str | None:
    ...
    if not match:
        return None
    a, b = (re.sub(r"\s+", " ", value).strip(" .,-–—") for value in match.groups())
    if not a or not b or not any(_fold(station) in _fold(f"{a} {b}") for station in STATIONS):
        return None
    return f"{a} – {b}"
...
class Line11StatusService:
    def __init__(self, settings: ApiSettings, *, transport: httpx.BaseTransport | None = None, clock: Any = time.monotonic) -> None:
        self.settings, self.transport, self.clock = settings, transport, clock
        self.lock, self.refresh_lock = threading.Lock(), threading.Lock()
        self.fresh: _Cache | None = None
        self.last_good: _Cache | None = None
        self.artesp: _Cache | None = None

    def _client(self) -> httpx.Client:
        return httpx.Client(timeout=httpx.Timeout(self.settings.transit_timeout_seconds, connect=min(5.0, self.settings.transit_timeout_seconds)), follow_redirects=True, transport=self.transport, headers={"User-Agent": "SuzanoAberta/1.0 (+https://github.com/MukaSanches/suzano-aberta)", "Accept": "text/html,application/json;q=0.95,*/*;q=0.7", "Accept-Language": "pt-BR,pt;q=0.9"})
        ...
        if current is None or current.get("_source_stale") or not current.get("operation_normal", False):
            try:
                ...
                if current is None or (artesp and self._prefer_artesp(current, artesp)):
                    ...
                    current = {**current, "occurrence": artesp["occurrence"], "affected_segment": artesp.get("affected_segment"), "reason": artesp.get("reason"), "secondary_source": artesp.get("source")}
            except Exception as exc:
                errors.append({"source": "ARTESP", "error": f"{type(exc).__name__}: {exc}"})
        if current is not None:
            out = copy.deepcopy(current)
            source_stale = bool(out.pop("_source_stale", False))
            out.update({"availability": "stale" if source_stale else "available", "stale": source_stale, "focus_stations": list(STATIONS), "checked_at": _iso(_now()), "source_errors": errors, "verified_sources": copy.deepcopy(list(VERIFIED_SOURCES)), "cache": {"state": "miss", "age_seconds": 0}})
    ...
    def _prefer_artesp(cptm: dict[str, Any], artesp: dict[str, Any]) -> bool:
        a, c = _dt(artesp.get("source_updated_at")), _dt(cptm.get("source_updated_at"))
    ...
    def _cptm(self) -> dict[str, Any]:
        ...
        raise TransitSourceUnavailable("; ".join(failures) or "CPTM não retornou estado utilizável")
    ...
    def _artesp_api(self) -> dict[str, Any]:
        ...
        with self._client() as client:
            ...
            line, operator = found
            status_obj = line.get("status") if isinstance(line.get("status"), dict) else {}
    ...
    def _html(self, html: str, source_name: str, source_url: str, source_id: str) -> dict[str, Any] | None:
...
def install_transit_routes(app: FastAPI, settings: ApiSettings) -> None:
    ...
    def line_11_status(response: Response) -> dict[str, Any]:
        payload = service.get()
        response.headers["Cache-Control"] = f"public, max-age={min(settings.transit_cache_seconds, 120)}, stale-while-revalidate=300" if payload.get("availability") == "available" else "public, max-age=30, stale-while-revalidate=120"
        return payload

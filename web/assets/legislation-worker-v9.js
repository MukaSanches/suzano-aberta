(() => {
  ...
  function sourceName(item) {
    ...
    return String(source.name || item?.source_name || "Fonte pública");
  }
  ...
  function prepare(item) {
    ...
    const source = fold(sourceName(item));
    const id = fold(item?.id);
    cached = {
      title,
      summary,
      source,
      text: `${title} ${summary} ${source} ${id}`.trim(),
  ...
  function matchQuery(item, clean, tokens) {
    if (!clean) return true;
    const text = prepare(item).text;
  ...
  function loadIndex() {
    if (!indexPromise) {
      indexPromise = fetchJson("../data/legislation-index.json?v=2").then(payload => {
        if (!payload?.meta) throw new Error("Índice legislativo inválido.");
        return payload.meta;
      });
    }
    return indexPromise;
  ...
  function loadShard(kind) {
    ...
    if (!shardPromises.has(kind)) {
      const promise = fetchJson(`../data/legislation-${kind}.json?v=2`).then(payload => {
        if (!Array.isArray(payload?.items)) throw new Error(`Shard ${kind} inválido.`);
        return payload.items;
      });
      shardPromises.set(kind, promise);
    }
  ...
  async function loadCandidates(kind) {
    ...
    const shards = await Promise.all(KINDS.map(loadShard));
  ...
  async function search(payload) {
    const [meta, candidates] = await Promise.all([
      loadIndex(),
      loadCandidates(String(payload.kind || "")),
    ]);

    const clean = fold(payload.query || "");
    const tokens = queryTokens(clean);
    const requestedYear = Number(payload.year || 0);
    const dateFrom = String(payload.date_from || "");
    const dateTo = String(payload.date_to || "");
    ...
    for (const item of candidates) {
      ...
      if (!matchQuery(item, clean, tokens)) continue;
      matches.push(item);
    }
    ...
    } else if (sort === "relevance" && clean) {
      matches = matches
        .map(item => ({ item, score: relevanceScore(item, clean, tokens) }))
        .sort((a, b) => {
          if (a.score !== b.score) return b.score - a.score;
          const ad = effectiveDate(a.item);
          const bd = effectiveDate(b.item);
          if (ad !== bd) return bd.localeCompare(ad);
    ...
    const limit = Math.max(1, Math.min(100, Number(payload.limit || PAGE_SIZE)));
    const offset = Math.max(0, Number(payload.offset || 0));
    return {
      total: matches.length,
      items: matches.slice(offset, offset + limit),
      meta,
      loaded_kind: payload.kind || "all",
    };
  }
  ...
  self.onmessage = async event => {
    ...
    try {
      const result = await search(event.data?.payload || {});
